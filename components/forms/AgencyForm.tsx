...
import { AgencyFormSchema } from "@/schemas";
...
const AgencyForm = ({ data }: Props) => {
  ...
  const handleSubmit = async (values: z.infer<typeof AgencyFormSchema>) => {
    try {
      ...
      if (!data?.customerId && !stripeCustomerId) return;

      if (data?.id) {
        await updateAgencyDetails(data.id, values);
        toast({
        ...
        const response = await createAgency({
          ...
          customerId: data?.customerId || stripeCustomerId || "",
